...
const fmt=n=>new Intl.NumberFormat("es-ES").format(Number.isFinite(n)?n:0);
const money=n=>new Intl.NumberFormat("es-ES",{style:"currency",currency:"EUR"}).format(Number.isFinite(n)?n:0);
const H=8;
async function safeCopy(t,el){try{if(typeof navigator!=="undefined"&&navigator.clipboard&&typeof navigator.clipboard.writeText==="function"){await navigator.clipboard.writeText(t);return"copied"}}catch(e){}
...
const PREMIUM={"01":2.6,"10":2.0,"20":3.25,"24":4.0,"25":3.6,"33":3.4,"41":5.0,"42":5.5,"43":6.0,"45":2.6,"46":1.9,"47":1.9,"49":3.4,"52":3.4,"55":2.25,"56":2.25,"62":1.0,"69":1.0,"81":3.25,"86":2.0};
...
// ---------- cálculo ----------
function splitDays(processes,avgD){
...
function calcImpact(company,absence,subs,groups){
  ...
  const typeCalc=(k,empCount,absPct,subRow,sal2)=>{
...
const Seg=({value,onChange,items})=>(
  <div className="inline-flex rounded-2xl border border-neutral-200 bg-white p-1 shadow-sm">{items.map(it=>(
    <button key={it.v} onClick={()=>onChange(it.v)} className={"rounded-2xl px-4 py-2 text-sm font-medium transition "+(value===it.v?"bg-neutral-900 text-white":"text-neutral-800 hover:bg-neutral-50")}>{it.t}</button>
  ))}</div>
);
const Card=({title,subtitle,right,children})=>(
  <section className="rounded-[28px] border border-neutral-200 bg-white/80 p-6 shadow-sm backdrop-blur">
    <div className="flex items-start justify-between gap-4"><div><div className="text-base font-semibold text-neutral-900">{title}</div>{subtitle?<div className="mt-1 text-sm text-neutral-600">{subtitle}</div>:null}</div>{right}</div>
    <div className="my-5 h-px w-full bg-gradient-to-r from-indigo-500/20 to-transparent" />
    {children}
  </section>
);
const Input=({label,right,hint,...p})=>(
  <label className="block"><div className="flex items-end justify-between gap-3"><div className="text-sm font-medium text-neutral-900">{label}</div>{right?<div className="text-xs text-neutral-500">{right}</div>:null}</div>
    <input {...p} className="mt-2 w-full rounded-2xl border border-neutral-200 bg-white px-4 py-3 text-sm text-neutral-900 shadow-sm outline-none transition focus:border-neutral-400 focus:ring-4 focus:ring-neutral-200" />
    {hint?<div className="mt-2 text-xs text-neutral-500">{hint}</div>:null}
  </label>
);
const Select=({label,value,onChange,children,hint})=>(
  <label className="block"><div className="text-sm font-medium text-neutral-900">{label}</div>
    <select value={value} onChange={e=>onChange(e.target.value)} className="mt-2 w-full rounded-2xl border border-neutral-200 bg-white px-4 py-3 text-sm text-neutral-900 shadow-sm outline-none transition focus:border-neutral-400 focus:ring-4 focus:ring-neutral-200">{children}</select>
    {hint?<div className="mt-2 text-xs text-neutral-500">{hint}</div>:null}
  </label>
);
const Toggle=({label,value,onChange})=>{
  const cls=value?"bg-neutral-900 text-white border-neutral-300":"bg-white text-neutral-900 border-neutral-200 hover:border-neutral-300";
  return(
    <button type="button" onClick={()=>onChange(!value)} className={"flex w-full items-center justify-between gap-3 rounded-2xl border px-4 py-3 text-sm shadow-sm transition "+cls}>
      <span className="font-medium">{label}</span><span className={"text-xs "+(value?"text-white/80":"text-neutral-500")}>{value?"Sí":"No"}</span>
    </button>
  );
};
const StatL=({label,value,sub})=>(
  <div className="rounded-[24px] border border-neutral-200 bg-white/80 p-4 shadow-sm"><div className="text-xs text-neutral-500">{label}</div>
    <div className="mt-1 text-xl font-semibold tracking-tight text-neutral-900">{value}</div>{sub?<div className="mt-1 text-xs text-neutral-500">{sub}</div>:null}
  </div>
);

function ExportPanel({open,json,onClose}){
  const ref=useRef(null);const[status,setStatus]=useState("ready");if(!open)return null;
        ...
        <div className="mt-4">
          ...
          <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <button onClick={copy} className="rounded-2xl bg-neutral-900 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-neutral-800">Copiar</button>
...
function SubTable({title,value,onChange}){
  const getRow=k=>(value&&value[k])?value[k]:emptySub();
  const setRow=(k,patch)=>{const cur=getRow(k);const next={...cur,...patch};const out={...(value||{})};out[k]=next;onChange(out)};
  return(
    <Card title={title} subtitle="% sustituidos y sobrecoste (overtime/ETT)" right={null}>
      <div className="overflow-x-auto">
        <table className="min-w-[820px] w-full border-separate border-spacing-y-2">
          ...
          <tbody>{TYPES.map(t=>{const row=getRow(t.k);return(
...
export default function AbsentismoApp(){
  ...
  const [absence,setAbsence]=useState({AT:1.2,IT_CORTA:2.5,IT_MEDIA:3.4,IT_LARGA:2.9});
  const [subs,setSubs]=useState({AT:{substitutedPct:30,extraCostPct:25},IT_CORTA:{substitutedPct:10,extraCostPct:20},IT_MEDIA:{substitutedPct:40,extraCostPct:30},IT_LARGA:{substitutedPct:55,extraCostPct:35}});
  ...
  const onCnaeChange=code=>{const next={...company,cnae:code};if(Object.prototype.hasOwnProperty.call(PREMIUM,code))next.ssVar=PREMIUM[code];setCompany(next)};
  ...
  const [gForm,setGForm]=useState({name:"",employees:0,salary:0,absence:{AT:0,IT_CORTA:0,IT_MEDIA:0,IT_LARGA:0},subs:{}});
  const addGroup=()=>{const name=String(gForm.name||"").trim();if(!name)return;const g={id:"g-"+String(Date.now()),name,employees:Math.max(0,Math.round(num(gForm.employees))),salary:Math.max(0,num(gForm.salary)),absence:{...gForm.absence},subs:gForm.subs||{}};setGroups([g].concat(groups));setGForm({name:"",employees:0,salary:0,absence:{AT:0,IT_CORTA:0,IT_MEDIA:0,IT_LARGA:0},subs:{}});setAddingGroup(false)};
  const delGroup=id=>setGroups(groups.filter(g=>g.id!==id));
    ...
    <main className="grid gap-6">
      <Card title="Empresa" subtitle="Datos base + SS + productividad" right={null}>
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Select label="CNAE" value={company.cnae} onChange={onCnaeChange} hint={cnaeHint}>
      ...
      <Card title="Grupos" subtitle="Especiales (con empleados)" right={
        <button onClick={()=>setAddingGroup(!addingGroup)} className={"rounded-2xl px-4 py-2 text-sm font-medium shadow-sm transition "+(addingGroup?"bg-neutral-900 text-white":"border border-neutral-200 bg-white text-neutral-900 hover:border-neutral-300")}>{addingGroup?"Cerrar alta":"+ Añadir"}</button>
      }>
        ...
        {addingGroup?(
          <div className="rounded-[26px] border border-neutral-200 bg-white p-5">
            ...
            <div className="mt-4 flex justify-end"><button onClick={addGroup} className="rounded-2xl bg-neutral-900 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-neutral-800">Guardar grupo</button></div>
          </div>
        ):null}
        {groups.length?(
      ...
      {!impact.ok?(<Card title="IMPACTO P&L" subtitle="No se puede calcular" right={null}><div className="text-sm text-rose-800">Empleados en grupos superior al total. Corrige en DATOS.</div></Card>):(
        <React.Fragment>
          <Card title="IMPACTO P&L" subtitle="Coste anual · Empresa vs Mutua" right={null}>
            <div className="rounded-[28px] border border-neutral-200 bg-white p-8 shadow-sm">
              ...
              <div className="mt-5 grid gap-3 md:grid-cols-3">
                ...
                <StatL label="Días de ausencia" value={fmt(impact.totals.days.toFixed(1))+" días"} sub={fmt(impact.totals.hours.toFixed(0))+" horas · tasa "+impact.absRate.toFixed(2)+"%"} />
              </div>
          ...
          <Card title="Detalle por tipología" subtitle="Empresa (directo + sustituciones + oportunidad si aplica)" right={null}>
            <div className="grid gap-3 md:grid-cols-2">{TYPES.map(t=>{const r=impact.byType[t.k];const w=impact.totals.companyTotal>0?(r.companyTotal/impact.totals.companyTotal)*100:0;return(
              <div key={t.k} className="rounded-[26px] border border-neutral-200 bg-white p-5 shadow-sm">
                <div className="flex items-start justify-between gap-3">
                  <div><div className="text-sm font-semibold text-neutral-900">{t.n}</div><div className="mt-1 text-xs text-neutral-500">{fmt(r.days.toFixed(1))} días · {fmt(r.hours.toFixed(0))}h</div></div>
        ...
        {!impact.ok?<div className="text-sm text-rose-800">Corrige asignación en DATOS.</div>:(
          <React.Fragment>
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <StatL label="Días de ausencia" value={fmt(impact.totals.days.toFixed(1))+" días"} sub={fmt(impact.totals.hours.toFixed(0))+" horas"} />
            </div>
  ...
  const views={datos:viewDatos,impacto:viewImpacto,informe:viewInforme};
      ...
      <div className="mx-auto max-w-6xl px-4 py-10">
        <header className="mb-8">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl font-semibold tracking-tight text-neutral-900">Calculadora de absentismo</h1>
              <p className="mt-2 text-sm text-neutral-600">DATOS · IMPACTO P&L · INFORME</p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <button onClick={()=>setExportOpen(true)} className="rounded-2xl border border-neutral-200 bg-white px-4 py-3 text-sm font-medium text-neutral-900 shadow-sm hover:border-neutral-300">Exportar JSON</button>
              <Seg value={tab} onChange={setTab} items={[{v:"datos",t:"DATOS"},{v:"impacto",t:"IMPACTO P&L"},{v:"informe",t:"INFORME"}]} />
            </div>
          </div>
          <div className="mt-6 flex flex-wrap gap-2">
            <span className="inline-flex items-center rounded-full border border-neutral-200 bg-white px-3 py-1 text-xs shadow-sm">Empleados: <b className="ml-1">{fmt(Math.max(0,Math.round(num(company.employees))))}</b></span>
            <span className="inline-flex items-center rounded-full border border-neutral-200 bg-white px-3 py-1 text-xs shadow-sm">En grupos: <b className="ml-1">{fmt(sumGroupEmp)}</b></span>
            <span className="inline-flex items-center rounded-full border border-neutral-200 bg-white px-3 py-1 text-xs shadow-sm">Resto: <b className="ml-1">{fmt(remainder)}</b></span>
