...
import ManagementDictionary from "../Management/ManagementDictionary";
...
const PURPLE_BG = "bg-gradient-to-r from-violet-600 to-indigo-600";
const PURPLE_RING = "ring-violet-200/50";
...
function makeCompany(seed = 7, companyName = "ACME"): Company {
  ...
  return { id: "co_1", name: companyName, provinces };
}
...
function Card(props: { title?: string; icon?: React.ReactNode; right?: React.ReactNode; children: React.ReactNode; className?: string }) {
  return (
    <div className={`rounded-2xl border border-zinc-100 bg-white shadow-[0_8px_30px_rgba(0,0,0,0.04)] transition-all hover:shadow-[0_8px_30px_rgba(0,0,0,0.08)] ${props.className || ""}`}>
      {(props.title || props.right) && (
        <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-50">
          <div className="flex items-center gap-2">
            {props.icon}
            {props.title && <div className="text-sm font-bold text-zinc-800 tracking-tight">{props.title}</div>}
          </div>
          {props.right}
        </div>
      )}
  ...
  return (
    <div className="w-full mt-4 select-none">
      ...
      <div className="relative h-32 w-full border-b border-slate-200">
        {/* Línea de umbral (Riesgo) - Ahora relativa a la altura total del contenedor de barras */}
        <div
          className="absolute left-0 right-0 border-t border-dashed border-rose-300 z-20 pointer-events-none"
          style={{ bottom: `${thresholdPct}%` }}
        >
          <span className="absolute right-0 -top-3 text-[9px] text-rose-400 bg-white/80 px-1 rounded font-bold">Riesgo {props.threshold}%</span>
        </div>

        {/* Contenedor de las barras (misma altura que el contenedor relativo para que los % coincidan) */}
              ...
              <div key={d.date} className="group relative flex-1 h-full flex items-end justify-center">
                ...
                <div
                  ...
                  className={`w-full rounded-t-md transition-all duration-300 relative z-10
                      ${isRisk ? "bg-rose-400/50" : "bg-emerald-400/50"} 
                      group-hover:opacity-100 group-hover:scale-x-110`}
                />

                {/* Etiqueta flotante (Tooltip) al pasar el ratón */}
                <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-[10px] py-1 px-2 rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity z-30 pointer-events-none shadow-xl flex flex-col items-center">
      ...
      <div className="h-6" /> {/* Espacio para las etiquetas de fecha */}
    </div>
  );
}
        ...
        <tbody className="divide-y divide-zinc-50">
          {props.rows.map((r, idx) => (
            <tr key={idx} className="hover:bg-zinc-50/50 transition-colors">
              {props.columns.map((c) => (
                <td key={c.key} className="px-4 py-3 align-middle text-zinc-700 tracking-tight">
                  {c.render(r)}
                </td>
              ))}
            </tr>
...
export default function Module1() {
  const storedName = getCompanyName();
  const company = useMemo(() => makeCompany(7, storedName), [storedName]);
  ...
  const [tab, setTab] = useState<"Empresa" | "Metodología" | "Reglas de riesgo" | "Diccionario Canónico">("Empresa");
    ...
    <div className="flex h-screen w-full flex-col bg-slate-50 font-sans text-slate-900">
      ...
      <header className="flex h-16 items-center justify-between border-b border-slate-200 bg-white px-6 shadow-sm z-10">
        ...
        <div className="flex items-center gap-4">
          <div className="hidden md:flex items-center gap-2 rounded-xl bg-zinc-100/50 p-1">
            {["Empresa", "Metodología", "Reglas de riesgo", "Diccionario Canónico"].map((t) => (
              <button
                key={t}
                onClick={() => setTab(t as any)}
                className={`rounded-lg px-3 py-1.5 text-xs font-bold transition-all tracking-tight ${tab === t ? "bg-white text-zinc-900 shadow-sm" : "text-zinc-500 hover:text-zinc-700"}`}
              >
                {t}
              </button>
            ))}
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto bg-slate-50/50 p-6">
          ...
          {tab === "Diccionario Canónico" && (
            <div className="h-full flex flex-col">
              <div className="mb-6">
                <h2 className="text-xl font-black text-zinc-900 tracking-tight">Diccionario Canónico</h2>
                <p className="text-xs text-zinc-500 font-medium">Define y estandariza los tipos de absentismo para toda la organización.</p>
              </div>
        ...
        {/* AI COPILOT PANEL */}
        <aside className="w-80 flex flex-col rounded-[28px] border border-zinc-200 bg-white shadow-sm m-4">
          <div className="flex items-center justify-between gap-3 p-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className="text-xs text-zinc-500">Tier-1 Advisor · No médico</div>
              </div>
            </div>
            <span className="inline-flex items-center rounded-full border border-violet-200 bg-violet-50 px-2.5 py-1 text-xs font-medium text-violet-700">Visible</span>
          </div>
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-4">
            <div className="flex gap-3">
              ...
              <div className="bg-zinc-100 rounded-2xl rounded-tl-none p-3 text-sm text-zinc-700 whitespace-pre-line">{aiA}</div>
            </div>
          </div>
          <div className="p-4 border-t border-zinc-100 bg-white rounded-b-[28px]">
            <div className="flex flex-wrap gap-2 mb-3">
              {["Qué riesgo tengo hoy", "Explícame la gráfica", "3 acciones para reducir"].map((t) => (
                <button
                  ...
                  className="text-[10px] bg-zinc-50 border border-zinc-200 rounded-full px-2 py-1 hover:bg-violet-50 hover:text-violet-700 transition-colors"
                >
                  {t}
                </button>
              ))}
            ...
            <div className="relative">
              <input
                ...
                className="w-full rounded-xl border border-zinc-200 bg-zinc-50 pl-3 pr-10 py-2 text-sm focus:ring-2 focus:ring-violet-200 focus:outline-none"
              />
              <button
                onClick={handleAiAsk}
