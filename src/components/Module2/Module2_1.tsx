...
/**
 * Módulo 2.1 · Home Impacto económico (versión corta y estable)
 * - KPIs con "Ver más" (personas / unidades)
 * - Desglose de costes (empresa / por unidad)
 * - Top focos + detalle
 * - Copiloto IA visible siempre
 *
 * Nota: demo sin datos médicos. Todo es configurable por empresa.
 */
...
// mini tests
void (() => {
  if (pct(0.1) !== "10.0%") console.warn("pct", pct(0.1));
  if (!eur(100).includes("")) console.warn("eur", eur(100));
  if (dmy("2026-01-02") !== "02/01/2026") console.warn("dmy", dmy("2026-01-02"));
})();

// --- Demo data (4 ejemplos) ---

const TOP_FOCI: FocusRow[] = [
  { unit: "Centro Norte", unitType: "Centro", role: "Operario", shift: "Mañana", eur: 96800, absRate: 0.081, trend: 0.14, critical: true },
  { unit: "Centro Sur", unitType: "Centro", role: "Operario", shift: "Tarde", eur: 74300, absRate: 0.069, trend: 0.09, critical: true },
  { unit: "Planta A", unitType: "Planta", role: "Operario", shift: "Noche", eur: 36900, absRate: 0.072, trend: 0.08, critical: true },
  { unit: "Tienda 14", unitType: "Tienda", role: "Supervisor", shift: "Finde", eur: 51200, absRate: 0.058, trend: 0.05, critical: true },
];

const RED_UNITS: RedUnitRow[] = [
  { unit: "Centro Norte", unitType: "Centro", critical: true, absentToday: 18, absentRate: 0.081, costMonth: 128000, riskScore: 92 },
  { unit: "Planta A", unitType: "Planta", critical: true, absentToday: 11, absentRate: 0.074, costMonth: 94000, riskScore: 88 },
  { unit: "Centro Sur", unitType: "Centro", critical: true, absentToday: 14, absentRate: 0.069, costMonth: 102000, riskScore: 84 },
  { unit: "Tienda 14", unitType: "Tienda", critical: true, absentToday: 6, absentRate: 0.058, costMonth: 51200, riskScore: 78 },
];

const ABSENCES: EmployeeRow[] = [
  { id: "E-10021", name: "Laura Santos", start: "2026-01-04", expected: "2026-01-28", costMonth: 1240, unit: "Centro Norte", dept: "Operaciones", gender: "F", role: "Operario", shift: "Mañana" },
  { id: "E-11002", name: "Pablo García", start: "2026-01-05", expected: "2026-01-23", costMonth: 1580, unit: "Planta A", dept: "Producción", gender: "M", role: "Técnico", shift: "Noche" },
  { id: "E-08818", name: "Sara Núñez", start: "2026-01-12", expected: "2026-01-25", costMonth: 740, unit: "Centro Sur", dept: "Operaciones", gender: "F", role: "Operario", shift: "Tarde" },
  { id: "E-12019", name: "Aina Vidal", start: "2026-01-09", expected: "2026-01-22", costMonth: 690, unit: "Tienda 14", dept: "Retail", gender: "F", role: "Supervisor", shift: "Finde" },
];
      ...
      <div className="flex items-center justify-between gap-3 p-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-xs text-zinc-500">Módulo 2 · 2.1</div>
          </div>
        </div>
...
export default function Module2_1_Home_Short() {
  const [modal, setModal] = useState<ModalState>({ kind: "none" });

  // filtros (solo afectan a listados)
  const [unitType, setUnitType] = useState<UnitType>("Centro");
  const [unitQuery, setUnitQuery] = useState("");
  const [criticalOnly, setCriticalOnly] = useState(false);

  // desglose de costes (empresa / unidad)
  const [scope, setScope] = useState<"empresa" | "unidad">("empresa");

  const kpi = useMemo(
    () => ({ absentToday: 0.062, redUnits: RED_UNITS.length, costMonth: 312000, costR12: 5680000 }),
    []
  );
  ...
  const filteredTop = useMemo(() => {
    ...
    return TOP_FOCI
      .filter((r) => (!criticalOnly ? true : r.critical))
      .filter((r) => r.unitType === unitType)
      .filter((r) => (!q ? true : r.unit.toLowerCase().includes(q)));
  }, [criticalOnly, unitQuery, unitType]);

  const filteredUnits = useMemo(() => {
    const q = unitQuery.trim().toLowerCase();
    return RED_UNITS.filter((u) => (!q ? true : u.unit.toLowerCase().includes(q)))
  ...
  const costLines = useMemo(() => {
    if (scope === "empresa") return COST_LINES;
    if (!unitQuery.trim()) return COST_LINES.map((x) => ({ ...x, eur: 0 }));
    // demo: si el usuario escribe una unidad, mostramos un 18% del total empresa
    return COST_LINES.map((x) => ({ ...x, eur: Math.round(x.eur * 0.18) }));
  }, [scope, unitQuery]);
  ...
  return (
    <div className="min-h-screen bg-white">
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_380px]">
          <div className="space-y-4">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <div className="flex items-center gap-2 rounded-2xl border border-zinc-200 bg-white px-3 py-2 shadow-sm">
                    ...
                    <input
                      ...
                      placeholder="Unidad (ej: Centro Norte)"
                  ...
                  <select
                    ...
                    title="Tipo de unidad (demo). En real: configurable por empresa."
                  >
                    {UNIT_TYPES.map((t) => (
                  ...
                  <Badge tone="violet">Afecta a listas</Badge>
                </div>
                <label className="flex items-center gap-3 rounded-2xl border border-zinc-200 bg-white px-3 py-2 shadow-sm">
                  <span className="text-sm font-medium text-zinc-700" title="Marcado por la empresa como crítico.">
                    Solo críticos
                  </span>
                  <input type="checkbox" checked={criticalOnly} onChange={(e) => setCriticalOnly(e.target.checked)} className="h-4 w-4 accent-violet-600" />
                </label>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
              <div className="rounded-[28px] border border-zinc-200 bg-white p-4 shadow-sm">
                <div className="flex items-center gap-2 text-xs font-medium text-zinc-600">
                  <Target className="h-4 w-4 text-violet-700" /> Hoy · % plantilla ausente <span className="text-zinc-400" title="Agregado del día">??</span>
                </div>
                <div className="mt-2 text-2xl font-semibold text-zinc-900">{pct(kpi.absentToday)}</div>
                <div className="mt-2 flex justify-end">
                  <button onClick={() => setModal({ kind: "absences" })} className="rounded-full border border-zinc-200 bg-white px-3 py-1.5 text-xs font-medium text-zinc-700 hover:bg-zinc-50" type="button">
              ...
              <div className="rounded-[28px] border border-zinc-200 bg-white p-4 shadow-sm">
                <div className="flex items-center gap-2 text-xs font-medium text-zinc-600">
                  <AlertTriangle className="h-4 w-4 text-violet-700" /> Unidades en rojo <span className="text-zinc-400" title="Fuera de umbral hoy">??</span>
                </div>
                <div className="mt-2 text-2xl font-semibold text-zinc-900">{kpi.redUnits}</div>
                <div className="mt-2 flex justify-end">
                  <button onClick={() => setModal({ kind: "units" })} className="rounded-full border border-zinc-200 bg-white px-3 py-1.5 text-xs font-medium text-zinc-700 hover:bg-zinc-50" type="button">
                    Ver más <ChevronRight className="ml-1 inline h-3.5 w-3.5" />
                  </button>
                </div>
              </div>
              ...
              <div className="rounded-[28px] border border-zinc-200 bg-white p-4 shadow-sm">
                <div className="flex items-center gap-2 text-xs font-medium text-zinc-600">
                  <PiggyBank className="h-4 w-4 text-violet-700" /> Coste mes (acumulado) <span className="text-zinc-400" title="Desde día 1 del mes">??</span>
                </div>
              ...
              <div className="rounded-[28px] border border-zinc-200 bg-white p-4 shadow-sm">
                <div className="flex items-center gap-2 text-xs font-medium text-zinc-600">
                  <PiggyBank className="h-4 w-4 text-violet-700" /> Coste anual móvil (12m) <span className="text-zinc-400" title="Últimos 12 meses">??</span>
                </div>
            ...
            <Card
              title={
                <span>
                  Costes asociados al absentismo <span className="text-zinc-400">(desglose)</span>
                </span>
              }
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="text-xs text-zinc-500">Conceptos y fórmulas configurables por empresa.</div>
              </div>
              ...
              <div className="mt-4 rounded-2xl border border-zinc-200 bg-zinc-50 p-3 text-sm text-zinc-800">
                ...
                <div className="mt-1 text-zinc-700">
                  Producción/servicio y oportunidad solo aparecen si la empresa define cómo monetizarlos.
                </div>
              </div>
            </Card>
            ...
            <Card
              title={
                <span>
                  Top focos de coste <span className="text-zinc-400">(ver detalle)</span>
                </span>
              }
      ...
      >
                ...
                <tbody>
                  {ABSENCES.map((e) => (
                    <tr key={e.id} className="border-t border-zinc-200 bg-white">
                      ...
                      <td className="p-3 text-zinc-700">{e.gender}</td>
        ...
        {modal.kind === "units" ? (
          <div className="space-y-3">
            <div className="text-sm text-zinc-700">Listado de unidades fuera de umbral hoy (foto del día).</div>
            <div className="overflow-x-auto rounded-2xl border border-zinc-200">
