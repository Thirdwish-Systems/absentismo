...
import { ABSENCE_DICTIONARY, AbsenceType } from './AbsenceDictionary';

export default function PredictionAbsenceTypesDetail() {
            ...
            <div className="lg:col-span-8 space-y-4">
                ...
                <div className="bg-white rounded-[32px] border border-zinc-100 overflow-hidden shadow-sm">
                    <div className="p-6 border-b border-zinc-50 flex items-center justify-between bg-zinc-50/30">
                        <div className="text-[10px] font-semibold text-zinc-400 uppercase tracking-widest">Tipos de absentismo previstos</div>
                        <div className="text-[10px] text-zinc-400 font-medium tracking-tight">Qué prevemos (IA) vs qué está ya programado</div>
                    </div>
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <div className="flex items-center gap-2">
                                            ...
                                            {!type.predecible && (
                                                <span className="px-2 py-0.5 rounded-full bg-zinc-100 text-zinc-500 text-[8px] font-bold uppercase tracking-widest border border-zinc-200">Calendario</span>
                                            )}
                                        </div>
                    ...
                    <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-[11px] text-zinc-400 mt-0.5">Asocia tus códigos nativos al modelo canónico de la plataforma.</p>
                            </div>
                        </div>
                        ...
                        <table className="w-full text-left text-xs">
                            ...
                            <tbody className="divide-y divide-zinc-50">
                                ...
                                <tr className="bg-amber-50/30">
                                    <td className="px-4 py-4 text-zinc-400 font-medium">Cegid (a3)</td>
                                    <td className="px-4 py-4 font-mono text-zinc-300">Code_550</td>
                                    <td className="px-4 py-4 text-zinc-300">Permiso Gremial</td>
                                    <td className="px-4 py-4 flex items-center gap-1 text-amber-600 font-bold"><AlertCircle className="h-3 w-3" /> Sin mapeo</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>

                    <button className="w-full mt-4 py-3 rounded-2xl border border-dashed border-zinc-200 text-zinc-400 text-xs font-medium hover:bg-zinc-50 transition-all flex items-center justify-center gap-2">
                        <FileSpreadsheet className="h-4 w-4" />
                        Cargar CSV de mapeo masivo
                    </button>
                </div>
            </div>
                ...
                <div className="sticky top-6 bg-white rounded-[32px] border border-zinc-100 p-8 shadow-sm">
                    {!selectedType ? (
                        <div className="h-[400px] flex flex-col items-center justify-center text-center p-6 bg-zinc-50/50 rounded-3xl border border-dashed border-zinc-200">
                            ...
                            <h5 className="text-sm font-bold text-zinc-400">Selecciona un tipo</h5>
                            <p className="text-[11px] text-zinc-300 mt-1">Haz clic en un elemento de la lista para ver su regla de cálculo y acciones.</p>
                        </div>
                    ) : (
                        <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
                            ...
                            <div className="p-5 rounded-2xl bg-zinc-50 border border-zinc-100">
                                ...
                                <div className="p-3 bg-white rounded-xl border border-zinc-200 font-mono text-[10px] text-zinc-600 break-all">
                                    {selectedType.rule}
                                </div>
                                <div className="mt-3 flex items-center gap-2 text-[10px] text-zinc-400">
                                    <Database className="h-3 w-3" />
                                    Fuente: <span className="text-zinc-900 font-bold">{selectedType.source}</span>
                                </div>
                            </div>

                            {selectedType.category === 'GESTIONABLE' && (
                                <div className="space-y-4">
                                    ...
                                    <button className="w-full py-3 rounded-2xl bg-violet-600 text-white text-xs font-bold shadow-sm shadow-violet-100 hover:bg-violet-700 transition-all flex items-center justify-center gap-2">
                                        Configurar workflow
                                    </button>
                                </div>
                            )}

                            <div className="p-4 rounded-2xl border border-zinc-100 text-[10px] leading-relaxed text-zinc-400">
                                <strong>Impacto en calidad:</strong> El dato se considera de calidad <span className="text-zinc-900">{selectedType.qualityImpact}</span>.
                                Requiere integración bidireccional con {selectedType.source.split(' / ')[0]}.
                            </div>
                        </div>
                    )}
                </div>
